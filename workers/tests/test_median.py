...
def execute_median_process(generate_data):
    def wrapped(data_arguments={}, ignore_nodata=None):
        ...
        return process.median.medianEOTask(None, "" , None).process(arguments)
    return wrapped
...
@pytest.mark.parametrize('data,attrs,expected_dims,expected_data', [
    ...
    ([[[[0.1, 0.15], [0.15, 0.2]], [[0.05, 0.1], [-0.9, 0.05]], [[0.2, 0.05], [-0.15, 0.1]]]], {'reduce_by': ['y']}, ('t','x','band'), [[[0.1, 0.1],[-0.15, 0.1]]]),
])
...
def test_with_xarray_nulls(execute_median_process, generate_data, data, expected_data, expected_dims, attrs, ignore_nodata):
    """
